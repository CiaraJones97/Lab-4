...
class CifarHelper():
    ...
    def set_up_images (self):
        ...
        # verticall stacks the training images
        ...
        # Reshape and normalizes test images
        ...
        self.training_images = np.vstack([d[b"data"]for d in self.all_train_batches])
        train_len = len (self.training_images)
        self.training_impages = self.training_images.reshape (train_len,3,32,32).transpose (0,2,3,1)/255
        self.training_labels = one_hot_encode(np.hstack([d[b"labels"] for d in self.all_train_batches]))
